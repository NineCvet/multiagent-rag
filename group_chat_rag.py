...
environmentalist_aid = RetrieveUserProxyAgent(
    ...
    retrieve_config={
        ...
        "get_or_create": True,
        "milvus_client": client,  # Connecting to Milvus server
    },
...
environmentalist_officer = AssistantAgent(
    ...
    is_termination_msg=termination_msg,
    llm_config=None,
    system_message="You oversee environmental policies, compliance with regulations, and the preparation of "
                   "environmental reports that detail the company's impact on the environment. Reply `TERMINATE` in "
                   "the end when everything is done.",
    description="Environmental Compliance Officer who answers to any question related to compliance with regulations, "
                "and the preparation of the environmental reports that detail the company's impact on the environment.",
)
...
def rag_chat():
    ...
    groupchat = autogen.GroupChat(
        agents=[environmentalist_aid, environmentalist_officer], messages=[], max_round=12,
        speaker_selection_method="round_robin"
    )
    manager = autogen.GroupChatManager(groupchat=groupchat, llm_config=llm_config)

    # Starting chatting with boss_aid as this is the user proxy agent.
...
def call_rag_chat():
    ...
    def retrieve_content(
            message: Annotated[
                str,
                "Refined message which keeps the original meaning and can be used to retrieve content for code "
                "generation and question answering.",
            ],
            n_results: Annotated[int, "number of results"] = 3,
    ) -> str:
        environmentalist_aid.n_results = n_results  # Set the number of results to be retrieved.
        # Checking if we need to update the context.
        ...
        if (update_context_case1 or update_context_case2) and environmentalist_aid.update_context:
            environmentalist_aid.problem = message \
                if not hasattr(environmentalist_aid, "problem") \
                else environmentalist_aid.problem
    ...
    # Disabling human input for boss_aid since it only retrieves content.
    environmentalist_aid.human_input_mode = "NEVER"
    ...
    # Starting chat with the boss as this is the user proxy agent.
    boss.initiate_chat(
        manager,
        message=PROBLEM,
    )
