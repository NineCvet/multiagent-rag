...
# Defining termination message function
...
google_env_expert = autogen.AssistantAgent(
    ...
    is_termination_msg=termination_msg,
    system_message="You are an expert at Google who has deep knowledge of environmental reports. Your role is to "
                   "analyze the Google Environmental Report 2022 and provide insights and recommendations based on "
                   "the report's content. Reply `TERMINATE` in the end when everything is done.",
    llm_config={"config_list": config_list_custom},
    description="Google Environmental Expert who can analyze the environmental report and provide expert insights and "
                "recommendations."
)
...
# Defining the group chat manager and agents
def _reset_agents():
    boss.reset()
    boss_aid.reset()
...
def rag_chat():
    ...
    # Starting chat with boss_aid
    boss_aid.initiate_chat(
        manager,
        message=boss_aid.message_generator,
        problem="Analyze the Google Environmental Report 2022 and provide insights into its environmental impact. "
                "Extract key data and suggest improvements.",
        n_results=3,
    )
